...
function getXML() {
  ...
  var resp = $fh.web({
    url: "http://www.freeflowdesign.net/filmforall/films.xml",
    method: "GET",
    charset: 'UTF-8',
  });

  var fileContents = resp.body;

  var xmlStart = fileContents.indexOf('?>') + 2;
  if (xmlStart > 0) {
    fileContents = fileContents.substring(xmlStart);
  }

  myJsonObject=xml2json.parser(fileContents);
    
  return {data:myJsonObject};
}
